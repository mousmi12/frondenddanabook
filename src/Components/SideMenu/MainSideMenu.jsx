...
function MainSideMenu({ menuLoaded, setMenuLoaded }) {
  ...
  useEffect(() => {
    dispatch(
      selectArabicAlignmentFunction({
        rightToLeft: loginresponse?.user_textrtl === "0" ? "ltr" : "rtl",
      })
    );
  }, []);
    ...
    <div className="h-screen py-3 w-[97%]">
      ...
      <div className="flex items-center justify-center">
        <img
          src={ZeroBookLogo}
          alt="ZeroBook"
          width={NavBarComponentStyle.zeroBookImgWidth}
          height={NavBarComponentStyle.zeroBookImgHeight}
        />
      </div>
      <div
        className="flex rounded-xl h-[95%] my-3 shadow sm:hidden"
        style={{
        ...
        <div className={`mt-[30%] w-[50%]`}>
          <div className="flex w-[95%] h-[100%] flex-col justify-start">
            {menuItems.menu &&
              menuItems.menu
                ...
                .map((item, index, array) => {
                  ...
                  const imgitem = svgImages.find(
                    (imgitem) => imgitem.name === item.image
                  );
                  // Check if the current item is a separator
                  const currentItemIsSeparator =
                    item.isseparator && !item.dockinbottom;
                  // Check if the previous item is a separator
                  const previousItemIsSeparator =
                    index > 0 &&
                    array[index - 1].isseparator &&
                    !array[index - 1].dockinbottom;
                      ...
                      !item.isseparator ? (
                        <button
                          ...
                          onClick={() => {
                            setFilterMenuId(item.id);
                            dispatch(
                              selectLabelFunction({ label: item.label })
                            );
                          }}
                        >
                          <img
                            ...
                            style={{
                              ...
                              filter: darkModeState.checkvalue
                                ? dayTheme.svgImageColor
                                : darkTheme.svgImageColor,
                            }}
                          />
                          <label
                            className="font-bold"
                            style={{
                              ...
                              color: darkModeState.checkvalue
                                ? dayTheme.menuLabelColor
                                : darkTheme.menuLabelColor,
                            }}
                          >
                            {item.label !== null
                        ...
                        currentItemIsSeparator &&
                        !previousItemIsSeparator && (
            ...
            <div className="mt-auto menubottomStyle py-2">
              {menuItems.menu &&
                menuItems.menu.some((item) => item.isseparator) &&
                        ...
                        <button
                          className={`${
                            item.id === filterMenuId
                              ? "buttonBgColorActiveMenu"
                              : "buttonColor"
                          }  hover:shadow rounded-r-2xl flex flex-col justify-center items-center w-[95%]`}
                          style={{ height: sideMenuDesign.buttonHeight }}
