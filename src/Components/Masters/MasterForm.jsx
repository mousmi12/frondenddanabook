...
import {
  ...
  handleDeleteApI,
  handleEnterKeyPress,
  handleEnterKeyPressMAaster,
} from "./masterPagefunctions";
...
function MasterForm() {
  ...
  const headeraccidInitialValue = formDetails?.form_fields
    ?.filter((field) => field.initialvalue !== null)
    .map((item) => item);
  const datasValues = headeraccidInitialValue?.reduce(
    (acc, item) => {
  ...
  const [getAccDataUpdate, setgetAccDataUpdate] = useState({
    billingaddress: {},
  });
  ...
  //this state for create mobile screen filter
  const [mobilescreenFilter, setMobilescreenFilter] = useState(null);

  // This Ref for create selectbox
  const selectboxAttributes = useRef(null);
  ...
  const chequeimagedetails = useSelector(
    (state) => state.getchequedata.chequedata
  );
  ...
  const clearForm = () => {
    if (selectState.name !== "Update") {
      const isFormDataMatching = formDetails?.form_fields?.some(
        (field) =>
          formData[field.name] == currentValue ||
          (formData[field.jsonref] &&
            formData[field.jsonref][field.name] == currentValue)
      );
      ...
      const isFormDataMatching = formDetails.form_fields.some(
        (field) =>
          ...
          (getAccDataUpdate.data[0][field.jsonref] &&
            getAccDataUpdate.data[0][field.jsonref][field.name] ===
              currentValue)
      );
    ...
    try {
      ...
      if (selectState.name === "Insert") {
        ...
        formDataWithJsonBillingAddress = {
          ...
          creditperiodexception: parseInt(
            getAccDataUpdate.data[0].creditperiodexception
          ),
          ...datasValues,
          billingaddress: JSON.stringify(
            getAccDataUpdate.data[0].billingaddress
          ),
        };
        if (parsedStoredRows && parsedStoredRows.length > 0) {
          formDataWithJsonBillingAddress.transaccounts = parsedStoredRows;
  ...
  return (
    ...
    color:${
      darkModeState.checkvalue
        ? dayTheme.TablabelTextColor
        : darkTheme.TablabelTextColor
    };
     background:${
       darkModeState.checkvalue
         ? dayTheme.mobileMenuFilterTabs
         : darkTheme.mobileMenuFilterTabs
     };
     border: solid 1px ${
       darkModeState.checkvalue
         ? dayTheme.mobileMenuFilterTabs
    ...
    background:${
      darkModeState.checkvalue
        ? dayTheme.mobileSavebutton
        : darkTheme.mobileSavebutton
    };
    border: solid 1px ${
      darkModeState.checkvalue
        ? dayTheme.mobileMenuFilterTabs
    ...
    `}
      ...
      {selectState.name !== null && selectState.opid !== null ? (
        <div className="flex items-start justify-center h-screen w-auto">
          {selectState.name === "List" ? (
            <MasterList
              formDetails={formDetails}
              hiddenReloadIcon={hiddenReloadIcon}
              sethiddenReloadIcon={sethiddenReloadIcon}
              LoadheadingTime={LoadheadingTime}
              setdisplayCustomAlertBox={setdisplayCustomAlertBox}
              setdisplaySuccErrAlert={setdisplaySuccErrAlert}
            />
            ...
            <div
              style={{
                background: darkModeState.checkvalue
                  ? dayTheme.DmenuCcolor
                  : darkTheme.DmenuCcolor,
              }}
              className={`w-full relative rounded sm:h-[84%] lg:h-[84%] xl:h-[84%] shadow pt-4 xl:mt-[95px] lg:mt-[95px] md:mt-[95px] sm:mt-[95px]`}
            >
              <div className="flex">
                <div
                  style={{
                    ...
                    color: darkModeState.checkvalue
                      ? dayTheme.DlabelTextColor
                      : darkTheme.DlabelTextColor,
                  }}
                  className="px-3 cursor-pointer"
                  onClick={() => setMobilescreenFilter(null)}
                >
                  {" "}
                  {formDetails?.name}
                </div>
                <div className="flex xl:hidden lg:hidden">
                  {formDetails.form_tabs &&
                    formDetails.form_tabs
                      ...
                      .map((item) => (
                        <button
                          ...
                          className={` flex justify-center mx-1 px-1 items-center text-center my-1 text-[${
                            masterFormDesign.textColor
                          }] rounded ${
                            item.id === mobilescreenFilter
                              ? "darkdayfilter"
                              : "darkdayfilterNot"
                          }`}
                          key={item.tabid}
              ...
              <form
                action=""
                onSubmit={
                  selectState.name !== "Delete" ? handleSave : deleteAlert
                }
              >
                <div
                  className={`${
                    formDetails?.form_tabs && formDetails?.form_tabs.length > 0
                      ? "lg:h-[40vh] lg:overflow-y-auto xl:h-[40vh] xl:overflow-y-auto"
                      : "sm:h-96 sm:overflow-y-auto lg:h-96 lg:overflow-y-auto xl:h-96 xl:overflow-y-auto"
                  }`}
                >
                  {formDetails?.form_fields &&
                    formDetails?.form_fields
                      .filter(
                        (filed) =>
                          filed.tabid === mobilescreenFilter &&
                          filed.visible === true
                      )
                      .sort((a, b) => a.position - b.position)
                      .map((field) => (
                        <div
                          key={field.id}
                          className={"flex sm:flex-col mt-[11px] sm:px-3"}
                        >
                          <div
                            className={`w-3/12 sm:hidden ${
                              arabicAlignMent.rightToLeft === "rtl"
                          ...
                          {["Text", "Email", "URL", "Number", "Date"].includes(
                            field.inputtype
                          ) && (
                              ...
                              <div className="relative sm:flex lg:w-[360px] xl:w-[360px]">
                                <input
                                  ...
                                  className={`${
                                    ...
                                    masterFormDesign.masterFormBorder
                                  } rounded-md w-full`}
                                  style={{
                                    height: `${masterFormDesign.inputTextHeight}`,
                                    width: `${masterFormDesign.inputTextWidth}`,
                                    backgroundColor: `${
                                      selectState.name === "Delete" ||
                                      selectState.name === "View" ||
                                      field.readonly
                                        ? dayTheme.inputFildReadColor
                                        : darkModeState.checkvalue
                                        ? dayTheme.inputFildColor
                                        : darkTheme.inputFildColor
                                    }`,
                                  }}
                                  readOnly={
                                    selectState.name === "Delete" ||
                                  ...
                                  value={
                                    selectState.name !== "Insert"
                                      ? getAccDataUpdate &&
                                        ...
                                        getAccDataUpdate.data.length > 0
                                        ? field.jsonref === null
                                          ? getAccDataUpdate.data[0][
                                              field.name
                                            ] || ""
                                          : (getAccDataUpdate.data[0][
                                              field.jsonref
                                ...
                                {isValidateRequired && (
                                  <p
                                    className={`text-rose-600 bold text-right absolute top-1  ${
                                      arabicAlignMent.rightToLeft === "rtl"
                                        ? "right-[280px] sm:right-[380px]"
                                        : "left-[280px] sm:left-[380px]"
                                    }`}
                                    style={{ fontSize: "10px" }}
                                  >
                                    {field.required == true ? (
                          ...
                          {field.inputtype === "Dropdown" && (
                            <SelectDropDown
                              ...
                              selectName={selectState.name}
                              getAccDataUpdate={getAccDataUpdate}
                              setgetAccDataUpdate={setgetAccDataUpdate}
                              setIsValidateRequired={setIsValidateRequired}
                              isValidateRequired={isValidateRequired}
                              setAccountList={setAccountList}
                              selectboxAttributes={selectboxAttributes}
                            />
                          )}
                        </div>
                      ))}
                ...
                <div className="xl:hidden lg:hidden flex justify-end absolute bottom-[70px] w-[100%] left-0 right-0">
                  <div className="px-1">
                    <div className="flex justify-end px-1">
                      {selectedSubMenuLabel.submenulabel ==
                        "Cheque Receipt" && (
                        <>
                          <input
                            type="file"
                    ...
                    <div>
                      <button
                        type="submit"
                        className={`m-2 px-1 darkdaysaveCancelNot rounded`}
                ...
                <div
                  className={`flex justify-between sm:hidden ${
                    darkModeState.checkvalue
                      ? dayTheme.DmenuCcolor
                      : darkTheme.DmenuCcolor
                  } footerShadow rounded-lg absolute bottom-0 w-[100%] left-0 right-0`}
                >
                  ...
                  <div>
                    <button
                      ...
                      className={`m-2 ${
                        saveButtonDisabled
                          ? `${masterFormDesign.buttonHoverColor}`
                          : ""
                      }`}
                      disabled={saveButtonDisabled}
                    >
                      <SimCardIcon
                        className={`hover:${masterFormDesign.buttonHoverColor}`}
                        style={{
                          width: "30px",
                          height: "30px",
                          color: darkModeState.checkvalue
                    ...
                    <button type="button" className={`m-2`} onClick={clearForm}>
                      <CloseIcon
                        className={`hover:text-[#43418e]`}
                        style={{
                          width: "30px",
                          height: "30px",
                          color: darkModeState.checkvalue
      ...
      {displayCustomAlertBox && (
        <AlertBoxMessage
          handleSave={handleSave}
          setdisplayCustomAlertBox={setdisplayCustomAlertBox}
          handeleDeleteFunction={handeleDeleteFunction}
        />
      )}
      {displaySuccErrAlert && (
        <AlertBoxSuccErr
      ...
      {chequeModal && (
        <ChequeModal
          ...
          handleChequeImage={handleChequeImage}
